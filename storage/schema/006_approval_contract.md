...
### Text correction values

หลัง action เป็น `correct` (ปุ่มหรือข้อความ `แก้ไข`) หรือเมื่อข้อความตรงรูปแบบแก้ไขโดยตรง
`intake_server` จะ parse ค่า override แล้วสร้าง draft revision ใหม่

รูปแบบที่รองรับ (ไม่ต้องมีคำว่า `แก้ไข` นำหน้า หากข้อความตรง pattern):

| กลุ่ม | ฟิลด์ | ตัวอย่าง |
| --- | --- | --- |
| สรุปบัตร | ผู้มีสิทธิ | `ผู้มีสิทธิ 1000` |
| สรุปบัตร | ผู้มาใช้สิทธิ | `ผู้มาใช้สิทธิ 900`, `ผู้ใช้สิทธิ 900` |
| สรุปบัตร | บัตรดี | `บัตรดี 850`, `ดี 850` |
| สรุปบัตร | บัตรเสีย | `บัตรเสีย 20`, `เสีย 20` |
| สรุปบัตร | ไม่ออกเสียง | `ไม่ออกเสียง 30`, `งด 30`, `vote no 30` |
| คะแนน | เบอร์ผู้สมัคร | `4=14`, `ผู้สมัคร 4=14`, `คะแนน 4=14` |
| เขต | area_id | `เขต 13` |

กติกาเพิ่มเติม:

- ตัวคั่นค่า: `=`, `เป็น`, `คือ`, `ควรเป็น`, `:`
- รองรับค่าก่อนชื่อฟิลด์ เช่น `850 บัตรดี`
- **ไม่รองรับ** การแมปเลขบรรทัดเอกสาร (`1=1000`, `2=900`, …)
- ข้อความสนทนาที่มีคำว่า `เบอร์` แต่ไม่มีรูปแบบคะแนน ไม่ถือเป็น correction

รายละเอียดเต็ม: [`docs/line-correction-parsing.md`](../../docs/line-correction-parsing.md)
